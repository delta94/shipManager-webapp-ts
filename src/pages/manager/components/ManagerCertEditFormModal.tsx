...
const ManagerCertEditFormModal: FC<ManagerCertEditFormModalProps> = ({ visible, current, onCancel, onSubmit }) => {
  ...
  return (
    ...
    >
      <Form {...formLayout} form={form} onFinish={handleFinish}>
        <Form.Item name="name" label="证书名" rules={[{ required: true, message: '请输入证书名称' }]}>
          <Input placeholder="请输入证书名称" />
        </Form.Item>
        <Form.Item name="identityNumber" label="证书编号" rules={[{ required: true, message: '请输入证书编号' }]}>
          <Input placeholder="请输入证书编号" />
        </Form.Item>

        <Form.Item name="expiredAt" label="证书过期时间" rules={[{ required: true, message: '请选择证书过期时间' }]}>
          <DatePicker placeholder="请选择证书过期日期" format="YYYY-MM-DD" style={{ width: '100%' }} />
        </Form.Item>

        <Form.Item name="typeId" label="证书类型" rules={[{ message: '请选择证书类型！', required: true }]}>
